...
std::string lap_result_to_json(const LapResult& r, bool full) {
    json j;
    ...
    if (full) {
        // ── Full mode: include the telemetry array ────────────────────────────
        ...
        // ── Summary mode: replace telemetry array with a point count ──────────
        j["telemetry_points"] = static_cast<int>(r.telemetry.size());
    }
